#!/usr/bin/env python3
"""
Complete workflow to create Canonical_Camargo dataset.
...
3. Create Canonical_Camargo dataset

Usage:
    # Run complete workflow
    python create_canonical_camargo.py \
    ...
    python create_canonical_camargo.py \
        --dataset-root "/Users/luorix/Desktop/MetaMobility Lab (CMU)/data/Final/Camargo" \
        --output-root "/Users/luorix/Desktop/MetaMobility Lab (CMU)/data/Final/Canonical_Camargo" \
        --subjects "AB06,AB07,AB08" --condition "levelground" --trial "LG_C0p0_S0p0_BT_1_10"
"""
...
def transform_trial_imu_data(
    ...
    try:
        ...
        for segment, segment_data in segments_data.items():
            ...
            if missing_cols:
                print(f"    ⚠️  Missing columns for {segment}: {missing_cols}")
                continue
            ...
            print(f"    ✓ Transformed {segment} IMU data")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Create Canonical_Camargo dataset")
    parser.add_argument("--dataset-root", required=True, help="Path to Final/Camargo dataset")
    parser.add_argument("--output-root", required=True, help="Path to output Canonical_Camargo dataset")
    ...
    parser.add_argument("--segments", default="all", help="Comma-separated segments")
    ...
    print(f"📋 Creating Canonical_Camargo dataset")
    ...
    print(f"\n🎉 Canonical_Camargo dataset creation complete!")
    print(f"   📁 Output dataset: {output_root}")
    if not args.skip_optimization:
        print(f"   📊 Optimization results: {results_dir / 'batch_results'}")
    else:
        print(f"   📊 Results used: {results_dir}")
    if args.dry_run:
