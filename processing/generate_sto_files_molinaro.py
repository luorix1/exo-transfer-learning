#!/usr/bin/env python3
"""
Generate .sto files for the Molinaro dataset from angle and moment CSV files.

This mirrors the output format and logic used by generate_sto_files_camargo.py and
generate_sto_files_keaton.py, producing one motion.sto per trial under the trial's
opensim folder, at the same depth as Input/Label.

Molinaro structure:
- /Volumes/Samsung_T5/raw_data/Molinaro/<subject>/<trial>/angle.csv (IK data)
- /Volumes/Samsung_T5/raw_data/Molinaro/<subject>/<trial>/moment.csv (ID data)

Usage:
  python generate_sto_files_molinaro.py \
    --input-root /Volumes/Samsung_T5/raw_data/Molinaro \
    --output-root "/Users/luorix/Desktop/MetaMobility Lab (CMU)/data/Final/Molinaro" \
...
def create_motion_sto_file(ik_df: pd.DataFrame, id_df: pd.DataFrame, output_path: Path) -> bool:
    ...
    try:
        ...
        # Normalize time column name (Molinaro files commonly have 'Header' or 'time')
        ...
        # Map Molinaro coordinate names to OpenSim names
        coord_mapping = {
            'hip_angle_l': 'hip_flexion_l',
            'knee_angle_l': 'knee_angle_l',
            'ankle_angle_l': 'ankle_angle_l',
            'hip_angle_r': 'hip_flexion_r',
        ...
        for coord in required_coords:
            if coord not in complete_df.columns:
                if coord.startswith('pelvis_t') and coord != 'pelvis_tilt':
                    ...
                    complete_df[coord] = 0.0
                elif coord in ['hip_adduction_r', 'hip_rotation_r', 'hip_adduction_l', 'hip_rotation_l']:
                    # Hip adduction/rotation (not available in Molinaro)
                    complete_df[coord] = 0.0
                elif coord in ['subtalar_angle_r', 'subtalar_angle_l']:
                    # Subtalar angles (not available in Molinaro)
                    complete_df[coord] = 0.0
                else:
...
def process_trial(angle_file: Path, moment_file: Path, trial_out_dir: Path) -> bool:
    try:
        ik_df = read_csv_flexible(angle_file)
        if ik_df is None:
            return False
        id_df = read_csv_flexible(moment_file)
        if id_df is None:
            return False
        ...
        if ok:
            print(f"  Created motion.sto for {angle_file.parent.name}")
        return ok
    except Exception as e:
        print(f"  Error processing trial {angle_file}: {e}")
        return False


def categorize_condition_from_trial_name(trial_name: str) -> str:
    """Categorize Molinaro trial to a condition directory.

    Heuristics:
    - Names starting with 'LG' -> levelground
    - Names containing 'incline' or 'ramp' -> ramp
    - Names containing 'stair' or 'stairs' -> stair
    Otherwise -> other
    """
    name = trial_name.lower()
    if name.startswith("lg"):
        return "levelground"
    if "incline" in name or "ramp" in name:
        return "ramp"
    if "stair" in name or "stairs" in name:
        return "stair"
    return "other"


def process_molinaro(input_root: str, output_root: str, conditions: List[str]) -> None:
    """Process Molinaro dataset and generate .sto files."""
    ...
    # Structure: <input>/<subject>/<trial>/(angle.csv, moment.csv)
    for subject_dir in in_root.iterdir():
        ...
        for trial_dir in subject_dir.iterdir():
            ...
            condition = categorize_condition_from_trial_name(trial_name)
            ...
            angle_file = trial_dir / "angle.csv"
            moment_file = trial_dir / "moment.csv"
            if not angle_file.exists() or not moment_file.exists():
                print(f"    Warning: Missing angle.csv or moment.csv in {trial_dir}")
                continue
...
def main():
    parser = argparse.ArgumentParser(description="Generate .sto files for Molinaro dataset")
    parser.add_argument("--input-root", required=True, help="Path to Molinaro raw data root (e.g., /Volumes/.../Molinaro)")
    parser.add_argument("--output-root", required=True, help="Path to Final Molinaro root")
    parser.add_argument("--conditions", default="levelground,ramp,stair", help="Comma-separated conditions")

    args = parser.parse_args()
    conditions = [c.strip() for c in args.conditions.split(',') if c.strip()]
    process_molinaro(args.input_root, args.output_root, conditions)
