...
def predict_on_trial(
    ...
    try:
        ...
        label_df = pd.read_csv(
            label_path, sep=None, engine="python", on_bad_lines="skip"
        )
    except:
...
def evaluate_model(
    ...
    if os.path.exists(config_path):
        ...
        if window_size is not None and window_size != config.get("window_size"):
            print(
                f"Overriding window_size: {config.get('window_size')} -> {window_size}"
            )
            config["window_size"] = window_size
    else:
        print(
            f"Config file not found at {config_path}, using default config with provided parameters"
        )
    ...
    ax.set_title("Hip Moment Prediction (Unilateral Model)", fontsize=14, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description="Test TCN model for joint moment prediction"
    )
    parser.add_argument(
